...
import styles from "./ListExamPart6.module.scss";
...
interface ListExamPart6Props {}
interface ListExamPart6State {
  allData: ToeicPart6Exam[];
}

export class ListExamPart6 extends BaseComponent<
  ListExamPart6Props,
  ListExamPart6State
> {
  constructor(props: ListExamPart6Props) {
    super(props);
    this.state = {
      allData: [],
    };
  ...
  async getAllData() {
    let allData = await toeicPart6ExamService.getAll<ToeicPart6Exam>(
      "ToeicPart6Exam",
      ""
    );
  ...
  renderItem(item: ToeicPart6Exam) {
    return (
      <Link to={`${ROUTER.EXAM_PART6}?keyDoc=${item.KeyDoc}`} key={item.KeyDoc}>
        <div className={styles.Contanier__leftcontent__item}>
          ...
          <div className={styles.Contanier__leftcontent__item__infor}>
            <div className={styles.Contanier__leftcontent__item__infor__title}>
              Đề luyện Toeic Part 6- Ets 2020 - {item.Title}- Có đáp án chi
              tiết
            </div>
    ...
    return (
      <div className={styles.Contanier}>
        <div className={styles.Contanier__leftcontent}>
          <h3>Series: Bộ đề thi Toeic Part 6 - Có đáp án chi tiết</h3>
