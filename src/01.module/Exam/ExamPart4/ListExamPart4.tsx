...
import styles from "./ListExamPart4.module.scss";
...
export class ListExamPart4 extends BaseComponent<
  ListExamPart4Props,
  ListExamPart4State
> {
  constructor(props: ListExamPart4Props) {
    super(props);
    this.state = {
      allData: [],
    };
  ...
  async getAllData() {
    let allData = await toeicPart4ExamService.getAll<ToeicPart4Exam>(
      "ToeicPart4Exam",
      ""
    );
  ...
  renderItem(item: ToeicPart4Exam) {
    return (
      <Link to={`${ROUTER.EXAM_PART4}?keyDoc=${item.KeyDoc}`} key={item.KeyDoc}>
        <div className={styles.Contanier__leftcontent__item}>
          ...
          <div className={styles.Contanier__leftcontent__item__infor}>
            <div className={styles.Contanier__leftcontent__item__infor__title}>
              Đề luyện Toeic Part 4 - Ets 2020 - {item.Title}- Có đáp án chi
              tiết
            </div>
